...
index = 84; %At what t index for peaks we want the inverse solve
...
[c1, c2] = get_constant(gamma, w_0, x_peaks(index), y_peaks(index)); %Use solution at time t (user determines time t using index) where a peak occurs

gam = sqrt(w_0^2 - gamma^2);

%defined analytical solution of data using c_1 and c_2
...
% hold on
% plot(full_X, full_y, 'k')
% plot(full_X, phi_an(full_X),'b')
% plot(x_peaks(index), y_peaks(index), 'mx')
...
% omega and gamma.
seg_start = round(N/2 + N/3);
t_seg = full_X(seg_start:end);   %going moderately far into the system     
y_seg = full_y(seg_start:end); 
...
phi_an = @(x) (exp(-gamma_hat.*x)).*(c1.*cos(sqrt(w_0_hat^2 - gamma_hat^2).*x) + c2.*sin(sqrt(w_0_hat^2 - gamma_hat^2).*x)); %define the solution in terms of c_1 and c_2
